...
public class ICTCLASDelegate
{
		...
		if (instance == null)
		{
			synchronized (ICTCLASDelegate.class)
			{
				if (instance == null)
				{
					...
					boolean test=instance.init();
					int test1=instance.importUserDictFile(getRootPath() + userDict);
					int test2=instance.ICTCLAS_SetPOSmap(0);
				}
			}
		}
		return instance;
	...
	public boolean init()
	{
		...
		return ictclas.ICTCLAS_Init(getRootPath().getBytes(defaultCharset));
	}
	...
	public int importUserDictFile(String path)
	{
		return ictclas.ICTCLAS_ImportUserDictFile(
			path.getBytes(defaultCharset), ECodeType.getECode(defaultCharset));
	}
	public int ICTCLAS_SetPOSmap(int label)
	{
		return ictclas.ICTCLAS_SetPOSmap(label);
	}
	...
	public String process(String source, int tagged)
	{
		byte[] buff = ictclas.ICTCLAS_ParagraphProcess(
			source.getBytes(defaultCharset),
			ECodeType.getECode(defaultCharset), tagged);
